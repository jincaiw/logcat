...
```bash
curl -O https://raw.githubusercontent.com/jincaiw/logcat/v0.2.11/docker-compose.yml
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/jincaiw/logcat/v0.2.11/scripts/install-linux.sh | sudo bash
```
...
```bash
tar -xzf logcat-0.2.11-linux-amd64.tar.gz
cd logcat-0.2.11-linux-amd64
./start.sh 8080
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/jincaiw/logcat/v0.2.11/scripts/install-linux.sh | sudo VERSION=0.2.11 bash
```
